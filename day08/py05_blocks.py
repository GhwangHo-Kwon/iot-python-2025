...
class Block:
    ...
    def move(self): # 볼 무브
        # 볼이 움직이는 x축 값을 계속 계산하려면 x축은 dir 값을 라디안으로 변환 후 코사인 처리
        self.rect.centerx += math.cos(math.radians(self.dir)) * self.speed
        # 볼이 움직이는 y축 값을 계속 계산하려면 y축은 dir 값을 라디안으로 변환 후 사인 처리
...
pygame.init()
Surface = pygame.display.set_mode((SCREAN_WIDTH, SCREAN_HEIGHT))
...
def main():
    ...
    colors = [(255, 0, 0), (255, 150, 0), (255, 228, 0),
              ...
              (201, 0, 167)]
    # 초기에 생성될 블럭들(무지개색으로 아홉개씩, 총 54개 블록)
    ...
    while True:
        ...
        for event in pygame.event.get(): # 이벤트처리 기본
            if event.type == QUIT:
                ...
                if event.key == K_LEFT:
                    ...
                    if PADDLE.rect.centerx > (SCREAN_WIDTH - 50):
                        PADDLE.rect.centerx = (SCREAN_WIDTH - 50)
                    else:
        ...
        if is_game_start == False:
            Surface.blit(M_GAME_TITLE, ((SCREAN_WIDTH / 2) - (400 / 2),
                                        (SCREAN_HEIGHT / 2) - (50 / 2)))
            Surface.blit(M_GAME_SUBTITLE, ((SCREAN_WIDTH / 2) - (300 / 2)
                                           , (SCREAN_HEIGHT / 2) + 50))
        else: # 게임시작 후 블록 다 그리고 볼이 움직이게 처리, 바도 움직이도록
            Surface.blit(M_SCORE, (10, 770))
            Surface.blit(M_SPEED, (SCREAN_WIDTH - 220, 770))
            ...
            if BALL.rect.centerx < 10 or BALL.rect.centerx > (SCREAN_WIDTH - 10): # 게임화면 양쪽 벽 밖으로 못나가게
            ...
            if len(BLOCK) == 0: # 볼로 블럭을 다 없앴음
                Surface.blit(M_CLEAR, ((SCREAN_WIDTH / 2) - (240 / 2),
                                    (SCREAN_HEIGHT / 2) - (50 / 2)))
                
            if BALL.rect.centery > 800:
                Surface.blit(M_FAIL, ((SCREAN_WIDTH / 2) - (240 / 2),
                                    (SCREAN_HEIGHT / 2) - (50 / 2)))
